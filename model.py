...
class GraphEmbed(nn.Module):
    def __init__(self, node_hidden_size):
        ...
        #embed graphs
        self.node_gating= nn.Sequential(nn.Linear(node_hidden_size,1),
                                        nn.Sigmoid())

        #graph representation from nodes
...
def bernoulli_action_log_prob(logit, action):
    #calculate lop p of action wrt bernoulli distribution
    if action==0:
...
class AddNode(nn.Module):
    def __init__(self, graph_embed_func, node_hidden_size):
        ...
        #if to initialize a node, initialize its hv
    ...
    def _initialize_node_repr(self, g, node_type, graph_embed):
        ...
        # print(num_nodes)
        #print(np.array(list(node_type)))
        # print(len(graph_embed))
        # print(graph_embed.shape)
        # print([node_type])
        #print(torch.LongTensor([node_type]))
        # print((torch.cat([
        #     self.node_type_embed(torch.LongTensor(np.array(list(node_type)))),graph_embed], dim=1)).shape)
        # print('continue')
        # hv_init = self.initialize_hv(torch.cat([
        #     self.node_type_embed(torch.LongTensor(np.array(list(node_type)))),graph_embed], dim=1))
        hv_init = self.initialize_hv(torch.cat([
            self.node_type_embed(torch.LongTensor([node_type])), graph_embed], dim=1))
...
class AddEdge(nn.Module):
    def __init__(self, graph_embed_func, node_hidden_size):
        ...
        #look eq 8 of paper, small change from add node
...
class DGMG(nn.Module):
    ...
    def forward_inference(self):
        ...
        while (not stop) and (self.g.number_of_nodes() < self.v_max + 1):
            ...
            while to_add_edge and (num_trails < self.g.number_of_nodes() - 1):
