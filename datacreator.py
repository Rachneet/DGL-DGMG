...
############################## our code for data generation for dgmg #################################

def orderNodes(G):
    node_order = list(range(G.number_of_nodes()))
    random.shuffle(node_order)
    order_mapping = dict(zip(G.nodes(), node_order))
    ...
    c_unused = 0
    nodes, edges, unused =[] , [], []
    for _,node in order_mapping.items():

        if len(nodes) == 0:
            nodes.append(node)
        both = [i for i in itertools.chain(nodes, edges)]

        nbrs = list(G.neighbors(node))
        check = any(item in both for item in nbrs)
        ...
        if not check and node not in nodes:
            unused.append(node)
    ...
    order = list(zip(nodes,edges))
    return order


def reorderNodes(edges):
    nodes = {}
    r_edges = []
    cnt = 0
    for u, v in edges:
        ...
        if v not in nodes:
            nodes[v] = cnt
            cnt += 1
        r_edges.append((nodes[u], nodes[v]))

    return r_edges


def getDecisionSequence(G):
    decision_sequence = []

    edges = orderNodes(G)

    # reorder the node indices from 0 - number of nodes so as to be input to the DGMG graph object

    edges = reorderNodes(edges)
    ...
    for u, v in edges:
        edge_cnt += 1
        if edge_cnt == 1:  # first edge
            decision_sequence.append(0)  # add first node
            nodes.append(u)
            decision_sequence.append(1)  # do not add edge
            decision_sequence.append(0)  # add second node
            nodes.append(v)
            decision_sequence.append(0)  # add edge
            decision_sequence.append(u)
            continue

        if u not in nodes:
            decision_sequence.append(1)  # stop adding edges from the previous round
            decision_sequence.append(0)  # add new node
            nodes.append(u)
        decision_sequence.append(0)  # add new edge
        decision_sequence.append(v)  # add destination
...
def generate_data(v_min, v_max, fname):
    samples = []
    for i in range(v_min, v_max + 1):
        for j in range(3, 4):
            for k in range(10):
                samples.append(getDecisionSequence(nx.barabasi_albert_graph(i, j)))
...
def generate_data_community(fname):
    ...
    graphs_train = utils.load_graph_list("/home/rachneet/PycharmProjects/graph_generation/graphs/GraphRNN_RNN_community2_4_128_train_0.dat")
...
#     for _ in range(n_samples):
#         num_edges = 3
#         samples.append(get_decision_sequence(num_edges))
#
#     # graphs = []
#     # for i in range(v_min, v_max+1):
#     #     for j in range(3, 4):
...
class BarabasiDataset(Dataset):
    def __init__(self,fname):
        super(BarabasiDataset, self).__init__()
...
class CommunityDataset(Dataset):
    def __init__(self,fname):
        super(CommunityDataset, self).__init__()
...
class CycleModelEvaluation(object):
    ...
    def write_summary(self):
        ...
        model_eval_path = os.path.join(self.dir, 'model_eval_community.txt')
...
class ModelEvaluation(object):
    ...
    def rollout_and_examine(self, model, num_samples):
        ...
        for i in range(num_samples):
            ...
            # print("Adj list to plot: ",len(adj_lists_to_plot))
            if len(adj_lists_to_plot) >= 4:
                plot_times += 1
                fig, ((ax0, ax1), (ax2, ax3)) = plt.subplots(2, 2)
                axes = {0: ax0, 1: ax1, 2: ax2, 3: ax3}
                for i in range(4):
                    nx.draw(nx.from_dict_of_lists(adj_lists_to_plot[i]),
                                     with_labels=True, ax=axes[i])
